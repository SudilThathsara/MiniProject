...
const Connections = () => {
  ...
  const { refreshCounts, notifications, markAsRead } = useNotifications()
  ...
  // Clear connection notifications when connections page loads
  useEffect(() => {
    const clearConnectionNotifications = async () => {
      const connectionNotifications = notifications.filter(n => n.type === 'connection' && !n.read);
      for (const notification of connectionNotifications) {
        await markAsRead(notification._id, 'connection');
      }
    };
