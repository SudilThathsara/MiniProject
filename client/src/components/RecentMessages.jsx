...
const RecentMessages = () => {
  ...
  return (
    <div className='bg-white max-w-xs mt-4 p-4 min-h-20 rounded-md shadow text-xs text-slate-800 border border-gray-200'>
      ...
      <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
        {messages.length === 0 ? (
          ...
          messages.map((message, index)=>(
            <Link to={`/messages/${message.from_user_id._id}`} key={index} className='flex items-start gap-2 py-2 hover:bg-slate-100 px-1 rounded'>
                ...
                <div className='w-full'>
                    ...
                    <div className='flex justify-between'>
                        <p className='text-gray-500 truncate max-w-32'>{message.text ? message.text : 'Media'}</p>
